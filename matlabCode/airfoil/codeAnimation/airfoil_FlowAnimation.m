...
% Save in avi file
v1 = VideoWriter('airfoil_streamAnimation.avi','Uncompressed AVI');
v2 = VideoWriter('airfoil_velocityAnimation.avi','Uncompressed AVI');
v3 = VideoWriter('airfoil_pressureAnimation.avi','Uncompressed AVI');
v4 = VideoWriter('airfoil_forcesAnimation.avi','Uncompressed AVI');
v1.FrameRate = 10;
v2.FrameRate = 10;
v3.FrameRate = 10;
v4.FrameRate = 10;
...
for k = 1 : nr_fr
    ...
    % AIRFOIL 
        ...
        % for shifting, rotating
            alpha = -pi/12 + (pi/6)*(k-1)/(nr_fr-1);    % rotating
        ...
        % velocity
            ...
            clf
            hold on
            quiver(xPlot,yPlot,uairPlot/40,vairPlot/40,0,'color',[0.4940 0.1840 0.5560],'LineWidth',1); %violet
            fill(xair,yair,'k','HandleVisibility','off')
        ...
        % forces
            ...
            hold on 
            fill(xair,yair,'k','HandleVisibility','off')
            quiver(xair,yair,fairx*5,fairy*5,0,'color',[0.3010 0.7450 0.9330],'LineWidth',1); %light blue
            legend('Force Field');
            axis equal;
            xlabel('x');
            ylabel('y');
...
for k = 1 : nr_fr
    ...
    % AIRFOIL 
        %-- PARAMETERS --------------------------------------------------------
        % for shifting, rotating
            alpha = -pi/12 + (pi/6)*(nr_fr-k)/(nr_fr-1);    % rotating
            beta = pi/18;   % y shifting
            e = 0.1;        % xshifting
            toll = 0.06;
        % general
